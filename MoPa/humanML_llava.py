...
class HumanMLDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, 
        ...
        self.catagories = ['Action','Body-Spatial','Temporal']
    ...
    def _discover_samples(self) -> List[Dict]:
        ...
        """
        # add 'Env 1' after self.split to match the directory structure
        
        split_dir = self.data_root / self.split / "Env1"
        ...
        for report_path in report_files:
            ...
            # get vqa json eg: sequence_000002_qna.json
            vqa_json = seq_dir / f"{seq_dir.name}_qna.json"
            # skip the sample if vqa json is not present
    ...
    def _read_vqa_json(self, vqa_json_path: str) -> List[Dict[str, Any]]:
        """Reads and returns VQA JSON data."""
        
        with open(vqa_json_path, 'r') as f:
            vqa_data = json.load(f)
            

        # For training, randomly select a category and then a question-answer pair within that category
        if self.split == 'train':
            selected_category = random.choice(self.catagories)
            category_pairs = [pair for pair in vqa_data['qa_pairs'] if pair['type'] == selected_category]
            if category_pairs:
                selected_pair = random.choice(category_pairs)
                return selected_pair['question'], selected_pair['answer']

        # For testing, return the first pair from the first category
        elif self.split in ['val', 'test']:
            #select the first category and the 1st question-answer pair in that category
            for category in self.catagories:
                category_pairs = [pair for pair in vqa_data['qa_pairs'] if pair['type'] == category]
                if category_pairs:
                    selected_pair = category_pairs[0]
                    return selected_pair['question'], selected_pair['answer']
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, str]:
        ...
        vqa_path = sample_info['vqa_json']
        question, answer = self._read_vqa_json(vqa_path)
        ...
        return seq_data_tensor, question, answer
...
def collate_fn(batch):
    ...
    answers = [item[2] for item in batch]  # List of strings
    return sequences, captions, answers