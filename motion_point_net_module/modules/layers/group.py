...
class QueryAndGroup(nn.Module):
    ...
    def forward(self, query_xyz: torch.Tensor, support_xyz: torch.Tensor, features: torch.Tensor = None) -> Tuple[
        ...
        extended_xyz = query_xyz.transpose(-2, -1).unsqueeze(-1).repeat(1, 1, 1, self.nsample)
        dist = self.pdist(extended_xyz, grouped_xyz)
        gaussion_density = torch.exp(- dist / (2.0 * self.radius * self.radius)) / (2.5 * self.radius)

        if self.normalize_dp:
            grouped_xyz /= self.radius

        grouped_xyz = torch.cat((
            grouped_xyz,
            gaussion_density.unsqueeze(-3)
        ), dim=-3)  # dim = 3+1
...
def get_aggregation_feautres(p, dp, f, fj, feature_type='dp_fj'):
    f = f.unsqueeze(-2) if fj.shape[-2] == 1 else f.unsqueeze(-1)
    if feature_type == 'dp_fj':
        fj = torch.cat([dp, fj], 1)
    elif feature_type == 'pi_dp_fj':
        fj = torch.cat([p.transpose(1, 2).unsqueeze(-1).expand(-1, -1, -1, dp.shape[-1]), dp, fj], 1)
