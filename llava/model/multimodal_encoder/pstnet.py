...
from pstnet_module.models import PSTnet


@dataclass
class PSTnetVisionConfig:
    """
    Configuration class for PSTnet motion encoder.
    """
    model_name: str = "pstnet-encoder"
    hidden_size: int = 512  # Output feature dimension from pstnet
    ...
    # PSTnet specific configs (will be loaded from checkpoint)
...
class PSTnetVisionTower(nn.Module):
    """
    Vision tower wrapper for PSTnet encoder.
    ...
    def __init__(self, vision_tower, args, delay_load=False):
        ...
        print(f"Loaded PSTnet config from {self.config_path}:")
        self.cfg_only = PSTnetVisionConfig(
            checkpoint_path=str(self.checkpoint_path),
            config_path=str(self.config_path),
            hidden_size=model_config['pipeline_args'].get('mlp_dim'),
    ...
    def load_model(self, device_map=None):
        """
        Load the PSTnet motion encoder from checkpoint.
        """
        if self.is_loaded:
            print(f'{self.vision_tower_name} is already loaded, `load_model` called again, skipping.')
            return
        
        print(f"Loading PSTnet motion encoder from: {self.checkpoint_dir}")
        ...
        print("Creating PSTnet pipeline...")
        self.vision_tower = PSTnet(feature_mode='all', **pipeline_args)
    ...
    def feature_select(self, motion_features):
        ...
        For PStnet with feature_mode='all', the output is already in the format we need:
        (batch, num_tokens, hidden_size)
        """
        # PStnet with feature_mode='all' returns: [batch, num_tokens, hidden_size]
    ...
    def forward(self, motion_data):
        """
        Forward pass through the PStnet motion encoder.
    ...
    def num_patches_per_side(self):
        ...
        For PStnet, this is derived from the ViT architecture.
        """
    ...
    def num_patches(self):
        ...
        For PStnet with feature_mode='all', this depends on the ViT configuration.
...
def build_pst_net_vision_tower(vision_tower, args, **kwargs):
    """
    Factory function to build the PStnet vision tower.
    This is called by the builder.
    """
    return PSTnetVisionTower(vision_tower, args=args, **kwargs)
