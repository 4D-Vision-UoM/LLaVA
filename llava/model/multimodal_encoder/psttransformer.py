"""
PSTTransformer Encoder for LLaVA
...
from pst_transformer_module.models import PSTTransformer


@dataclass
class PSTTransformerVisionConfig:
    """
    Configuration class for PSTTransformer motion encoder.
    """
    model_name: str = "psttransformer-encoder"
    hidden_size: int = 80  # Output feature dimension from psttransformer
    ...
    # PSTTransformer specific configs (will be loaded from checkpoint)
...
class PSTTransformerVisionTower(nn.Module):
    """
    Vision tower wrapper for PSTTransformer encoder.
    ...
    def __init__(self, vision_tower, args, delay_load=False):
        """
        Args:
            vision_tower: str, path to checkpoint directory (e.g., "PSTTransformer/ckpt/HumanML_PSTTransformer_32_frames_64batch")
        ...
        print(f"Loaded PSTTransformer config from {self.config_path}:")
        self.cfg_only = PSTTransformerVisionConfig(
    ...
    def load_model(self, device_map=None):
        """
        Load the PSTTransformer motion encoder from checkpoint.
        """
        if self.is_loaded:
            print(f'{self.vision_tower_name} is already loaded, `load_model` called again, skipping.')
            return
        
        print(f"Loading PSTTransformer motion encoder from: {self.checkpoint_dir}")
        ...
        print("Creating PSTTransformer pipeline...")
        self.vision_tower = PSTTransformer(feature_mode='all', **pipeline_args)
        ...
        print(f"✓ PSTTransformer encoder loaded:")
...
def build_motion_point_net_vision_tower(vision_tower, args, **kwargs):
    ...
    """
    return PSTTransformerVisionTower(vision_tower, args=args, **kwargs)
