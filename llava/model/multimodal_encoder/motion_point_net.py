"""
MotionPointnet Encoder for LLaVA
...
from motion_point_net_module.models import MotionPointNet


@dataclass
class MotionPointNetVisionConfig:
    """
    Configuration class for MotionPointNet motion encoder.
    """
    model_name: str = "motionpointnet-encoder"
    hidden_size: int = 1024  # Output feature dimension from motionpointnet
    ...
    # Moionpointnet specific configs (will be loaded from checkpoint)
...
class MotionPointNetVisionTower(nn.Module):
    """
    Vision tower wrapper for MotionPointNet encoder.
    ...
    def __init__(self, vision_tower, args, delay_load=False):
        """
        Args:
            vision_tower: str, path to checkpoint directory (e.g., "MotionPointNet/ckpt/HumanML_MotionPointNet_32_frames_64batch")
        ...
        print(f"Loaded MotionPointNet config from {self.config_path}:")
        self.cfg_only = MotionPointNetVisionConfig(
            checkpoint_path=str(self.checkpoint_path),
            config_path=str(self.config_path),
            hidden_size=model_config['pipeline_args'].get('hidden_dim'),
    ...
    def load_model(self, device_map=None):
        """
        Load the MotionPointNet motion encoder from checkpoint.
        """
        if self.is_loaded:
            print(f'{self.vision_tower_name} is already loaded, `load_model` called again, skipping.')
            return
        
        print(f"Loading MotionPointNet motion encoder from: {self.checkpoint_dir}")
        ...
        print("Creating MotionPointNet pipeline...")
        self.vision_tower = MotionPointNet(feature_mode='all', **pipeline_args)
        ...
        print(f"✓ MotionPointNet encoder loaded:")
    ...
    def feature_select(self, motion_features):
        ...
        For MotionPointNet with feature_mode='all', the output is already in the format we need:
        (batch, num_tokens, hidden_size)
        """
        # MotionPointNet with feature_mode='all' returns: [batch, num_tokens, hidden_size]
    ...
    def forward(self, motion_data):
        """
        Forward pass through the MotionPointNet motion encoder.
    ...
    def num_patches_per_side(self):
        ...
        For MotionPointNet, this is derived from the ViT architecture.
        """
    ...
    def num_patches(self):
        ...
        For MotionPointNet with feature_mode='all', this depends on the ViT configuration.
...
def build_motion_point_net_vision_tower(vision_tower, args, **kwargs):
    """
    Factory function to build the MotionPointNet vision tower.
    This is called by the builder.
    """
    return MotionPointNetVisionTower(vision_tower, args=args, **kwargs)
