...
class PSTTransformer(nn.Module):
    def __init__(self, radius, nsamples, spatial_stride,                                # P4DConv: spatial
                 temporal_kernel_size, temporal_stride,                                 # P4DConv: temporal
                 dim, depth, heads, dim_head, dropout1,                                 # transformer
                 mlp_dim,head_type,projection_dim,device,feature_mode='all'):                                       # output
        super().__init__()
    ...
    def forward(self, batch):                                                                                                               # [B, L, N, 3]
        ...
        output = self.transformer(xyzs, features)
        output = torch.max(input=output, dim=1, keepdim=False, out=None)[0] # for the sake of llava training we get multiple tokens 
        # print(f"After first temporal max pooling, output shape: {output.shape}")  # Debugging line
        # output = torch.max(input=output, dim=1, keepdim=False, out=None)[0]
